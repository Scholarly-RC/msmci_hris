...
def get_biometric_detail_from_user_id(user_id):
    ...
    return BiometricDetailModel.objects.filter(user__id=user_id).first()
...
def _get_biometric_detail_from_device_user_id(device_user_id):
    ...
    """
    BiometricDetailModel = apps.get_model("core", "BiometricDetail")
    return BiometricDetailModel.objects.filter(user_id_in_device=device_user_id).first()
...
def _get_punch_value(value):
    ...
    if value == 0:
        return AttendanceRecordModel.Punch.TIME_IN.value
